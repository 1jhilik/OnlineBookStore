...
if(isset($_POST['submit'])){
   ...
   if($user_id != ''){
      ...
      $verify_review = $con->prepare("SELECT * FROM `reviews` WHERE prod_id = ? AND user_id = ?");
      $verify_review->execute([$get_id, $user_id]);

      if($verify_review->rowCount() > 0){
         $warning_msg[] = 'Your review already added!';
      }else{
         $add_review = $con->prepare("INSERT INTO `reviews`(id, prod_id, user_id, rating, title, description) VALUES(?,?,?,?,?,?)");
